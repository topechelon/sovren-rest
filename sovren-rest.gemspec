# Generated by juwelier
# DO NOT EDIT THIS FILE DIRECTLY
# Instead, edit Juwelier::Tasks in Rakefile, and run 'rake gemspec'
# -*- encoding: utf-8 -*-

# stub: sovren-rest 0.1.0 ruby lib

Gem::Specification.new do |s|
  s.name = 'sovren-rest'.freeze
  s.version = '0.1.0'

  s.required_rubygems_version = Gem::Requirement.new('>= 0'.freeze) if s.respond_to? :required_rubygems_version=
  s.require_paths = ['lib'.freeze]
  s.authors = ['TEN Devs V3'.freeze]
  s.date = '2018-10-16'
  s.description = 'Interfaces with the Sovren 9.0 REST API'.freeze
  s.email = 'tendevsv3@patriotsoftware.com'.freeze
  s.extra_rdoc_files = [
    'LICENSE.txt',
    'README.rdoc'
  ]
  s.files = [
    '.document',
    '.rspec',
    '.rubocop.yml',
    'Gemfile',
    'Gemfile.lock',
    'LICENSE.txt',
    'README.rdoc',
    'Rakefile',
    'VERSION',
    'lib/address.rb',
    'lib/certification.rb',
    'lib/client.rb',
    'lib/contact_information.rb',
    'lib/education_history.rb',
    'lib/employment_history.rb',
    'lib/experience_summary.rb',
    'lib/resume.rb',
    'lib/sovren_rest.rb',
    'sovren-rest.gemspec',
    'spec/integration/files/resume.pdf',
    'spec/integration/parse_spec.rb',
    'spec/spec_helper.rb',
    'spec/unit/certification_spec.rb',
    'spec/unit/client_spec.rb',
    'spec/unit/contact_info_spec.rb',
    'spec/unit/education_history_spec.rb',
    'spec/unit/employment_history_spec.rb',
    'spec/unit/experience_summary_spec.rb',
    'spec/unit/files/certification.json',
    'spec/unit/files/contact-info.json',
    'spec/unit/files/education-history.json',
    'spec/unit/files/employment-history.json',
    'spec/unit/files/experience-summary.json',
    'spec/unit/files/resume.json',
    'spec/unit/resume_spec.rb'
  ]
  s.homepage = 'http://github.com/SynergyDataSystems/sovren-rest'.freeze
  s.licenses = ['MIT'.freeze]
  s.rubygems_version = '2.5.2.3'.freeze
  s.summary = 'Sovren 9.0 Rest'.freeze

  if s.respond_to? :specification_version
    s.specification_version = 4

    if Gem::Version.new(Gem::VERSION) >= Gem::Version.new('1.2.0')
      s.add_runtime_dependency('json'.freeze, ['>= 0'])
      s.add_runtime_dependency('rest-client'.freeze, ['>= 0'])
      s.add_development_dependency('bundler'.freeze, ['~> 1.0'])
      s.add_development_dependency('byebug'.freeze, ['>= 0'])
      s.add_development_dependency('juwelier'.freeze, ['~> 2.1.0'])
      s.add_development_dependency('rdoc'.freeze, ['~> 3.12'])
      s.add_development_dependency('rspec'.freeze, ['>= 0'])
      s.add_development_dependency('rubocop'.freeze, ['>= 0'])
      s.add_development_dependency('simplecov'.freeze, ['>= 0'])
      s.add_development_dependency('simplecov-rcov'.freeze, ['>= 0'])
    else
      s.add_dependency('bundler'.freeze, ['~> 1.0'])
      s.add_dependency('byebug'.freeze, ['>= 0'])
      s.add_dependency('json'.freeze, ['>= 0'])
      s.add_dependency('juwelier'.freeze, ['~> 2.1.0'])
      s.add_dependency('rdoc'.freeze, ['~> 3.12'])
      s.add_dependency('rest-client'.freeze, ['>= 0'])
      s.add_dependency('rspec'.freeze, ['>= 0'])
      s.add_dependency('rubocop'.freeze, ['>= 0'])
      s.add_dependency('simplecov'.freeze, ['>= 0'])
      s.add_dependency('simplecov-rcov'.freeze, ['>= 0'])
    end
  else
    s.add_dependency('json'.freeze, ['>= 0'])
    s.add_dependency('rest-client'.freeze, ['>= 0'])
    s.add_dependency('rest-client'.freeze, ['>= 0'])
    s.add_dependency('rest-client'.freeze, ['>= 0'])
    s.add_dependency('rest-client'.freeze, ['>= 0'])
    s.add_dependency('rest-client'.freeze, ['>= 0'])
    s.add_dependency('rspec'.freeze, ['>= 0'])
    s.add_dependency('rubocop'.freeze, ['>= 0'])
    s.add_dependency('simplecov'.freeze, ['>= 0'])
    s.add_dependency('simplecov-rcov'.freeze, ['>= 0'])
  end
end
