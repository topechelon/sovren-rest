...
RSpec.describe SovrenRest::Client do
  ...
  let(:default_additional_parse_request_headers) { {} }
  ...
  describe 'initialization' do
    ...
    it 'defaults the additional_parse_request_headers if not specified' do
      expect(client.instance_variable_get(:@additional_parse_request_headers)).to eq(default_additional_parse_request_headers)
    end
  end
  ...
  describe 'parse' do
    ...
    context 'when additional headers are supplied' do
      ...
      before do
        client.instance_variable_set(:@additional_parse_request_headers, { 'x-api-key' => '123456', 'Content-Type' => 'text/plain' })
      end
    ...
    context 'unsuccessful post' do
      ...
      context 'misc exceptions' do
        ...
        context 'when response body cannot be parsed' do
          ...
          let(:response_parse_error) { SovrenRest::ClientException::ResponseParseError.new(raw_post_response_body) }
          ...
          it 'raises a SovrenRest::ClientException::ResponseParseError exception' do
            expect { subject }.to raise_error(response_parse_error)
          end
        end
      end
    end
